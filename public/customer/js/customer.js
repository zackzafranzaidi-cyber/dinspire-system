      ...
      prodGrid.innerHTML = filtered.map(
        (p) => {
          ...
          if (isOutOfStock) {
              ...
              stockBadge = `<div style="position:absolute; top:8px; right:8px; background:rgba(0,0,0,0.65); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); color:#fff; font-size:9.5px; padding:4px 8px; border-radius:8px; font-weight:700; z-index:11; display:flex; align-items:center; gap:4px; border: 1px solid rgba(255,255,255,0.15); box-shadow: 0 4px 10px rgba(0,0,0,0.2);"><i class="fas fa-box-open text-purple-400"></i> Stok: ${stockLeft}</div>`;
          }
        ...
        <div style="position: relative; border-bottom: 1px solid var(--border-color); overflow: hidden; min-height: 56px; margin: 0 -16px;">
            ...
            <div id="swipe-content-${id}" 
                ...
                <div class="qty-control flex items-center justify-between bg-gray-100 rounded-lg p-1" style="width:70px; flex:none;">
                    <button type="button" class="qty-btn w-6 h-6 rounded  font-bold" style="background: var(--bg-surface);" onclick="updateEditCartQty('${id}', -1)">-</button>
...
async function fetchBarberAvailabilityForReset() {
  ...
  try {
    ...
    let allTimes = ["10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30", "22:00", "22:30"];
