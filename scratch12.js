...
const newUpdateOwnerBadges = `// UPDATE OWNER BADGES
function updateOwnerBadges() {
    if (typeof masterData === "undefined" || !masterData) return;
    
    let totalServis = masterData.bookings ? masterData.bookings.length : 0;
    let seenServis = parseInt(localStorage.getItem('din_seen_tx_servis_count')) || 0;
    window.newSelesaiCount = Math.max(0, totalServis - seenServis);
...
code = code.replace(/\/\/ UPDATE OWNER BADGES\s*function updateOwnerBadges\(\)\s*\{[\s\S]*?(?=\n\s*function adjustAILayout)/, newUpdateOwnerBadges + '\n');
