...
const regex = /function updateCartUI\(\) \{[\s\S]*?\}\s*\}/;
...
js = js.replace(/function updateCartUI\(\) \{[\s\S]*?\}\s*\}/, match => match.replace(/}$/, '  renderDesktopCartItems();\n}'));

js += injection;
fs.writeFileSync('public/js/index.js', js);
console.log("Injected renderDesktopCartItems.");
