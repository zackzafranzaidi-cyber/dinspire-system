...
let mapFnStart = code.indexOf('const mapOrderToHTML');
let mapFnEnd = code.indexOf('};', mapFnStart);

let prefix = code.slice(0, mapFnStart);
let suffix = code.slice(mapFnEnd);
let mid = code.slice(mapFnStart, mapFnEnd);

mid = mid.replace(/return `[\s\S]*?<\/tr>`;/, replacement);
